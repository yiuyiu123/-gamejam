...
public class PlayerCamera : MonoBehaviour
{
    ...
    [Header("垂直偏移设置")]
    public float verticalOffset = 2f; // 垂直偏移量，让玩家在屏幕靠下位置
    public bool enableVerticalOffset = true; // 是否启用垂直偏移
    ...
    void LookAtTarget()
    {
        ...
        {
            targetPoint += Vector3.up * verticalOffset;
        }
        ...
        if (Time.frameCount % 120 == 0)
        {
            ...
            float angle = Vector3.Angle(transform.forward, directionToTarget);
            Debug.Log($"摄像机看向目标 - 角度差: {angle:F2}°, 垂直偏移: {verticalOffset}");
        }
    }
    ...
    void OnDrawGizmosSelected()
    {
        ...
        if (enableVerticalOffset)
        {
            lookTarget += Vector3.up * verticalOffset;
            // 显示偏移点
            Gizmos.color = Color.yellow;
    ...
    {
        Debug.Log("=== 摄像机状态 ===");
        Debug.Log($"目标: {(target != null ? target.name : "无")}");
        Debug.Log($"当前角度: {currentAngle:F1}°");
        Debug.Log($"垂直偏移: {verticalOffset}");
        Debug.Log($"启用垂直偏移: {enableVerticalOffset}");
        Debug.Log($"摄像机位置: {transform.position}");
        Debug.Log($"摄像机旋转: {transform.rotation.eulerAngles}");
        Debug.Log("=================");
    }
}
